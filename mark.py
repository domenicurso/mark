...
try:
    from Cocoa import NSObject, NSWorkspace, NSLog
    MACOS_SUPPORT = True
except ImportError:
    MACOS_SUPPORT = False
...
from utils.constants import VERSION, MIN_RATE_LIMIT, FB_ICON, FB_TEXT, l
...
class ShutdownObserver(object):
    ...
    def workspaceWillPowerOff_(self, notification):
        if MACOS_SUPPORT:
            NSLog("Received NSWorkspaceWillPowerOffNotification. Initiating graceful shutdown.")
        else:
...
class MarkApp:
    ...
    def load_settings(self, settings_path=None):
        ...
        try:
            ...
            self.retry_interval = max(MIN_RATE_LIMIT, self.settings.get("retry_interval", 0))
            return True
    ...
    def graceful_shutdown(self):
        ...
        if remaining_time > 0:
            l.info(f"Waiting {remaining_time:.2f} seconds before final status reset to avoid rate limits...")
    ...
    def run(self, fast=False, debug=False):
        ...
        try:
            ...
            if debug:
                l.debug(f"Starting Mark with fast mode {'enabled' if fast else 'disabled'} and debug mode enabled.")
    ...
    def status_update_loop(self, debug=False):
        ...
        while True:
            ...
            if not self.plugin_manager or not self.status_manager:
                l.error("Plugin manager or status manager not initialized. Retrying in 5 seconds...")
                time.sleep(5)
                continue
            ...
            try:
                ...
                context = {
                    ...
                    "_idle": get_idle_time(),
                    "_enabled": self.settings.get("statuses", {}).get("plugins", {}).get("_enabled", [])
                }
                ...
                if (current_status != self.status_manager.last_status and 
                    (now - self.status_manager.last_status_time) > self.update_interval):
                    
                    if debug:
                        l.debug("Updating Discord status to: " + emoji + " " + text + " (type: " + status_type + ")")
                        
                    self.status_manager.set_custom_status(emoji, text, status_type)
                    
                    if debug:
                        l.success("Updated Discord status to: " + emoji + " " + text + " (type: " + status_type + ")")
                elif debug:
...
@app.alias("fast", "f")
@app.alias("verbose", "v")
def run(fast: bool = False, verbose: bool = False):
    """Initialize Mark"""
    mark = MarkApp()
...
@app.command
def version():
    """Get the version of Mark"""
    return f"Mark v{VERSION}"


@app.command
def gb():
    """Shortcut for getbundle command"""
    app.execute("getbundle")
